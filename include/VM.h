...
private:
    ...
    inline void CALL_LOCAL(){                           // IMPLEMENT ME
        ...
        if(m_stack.size() < (int)(m_data[m_memptr]))
            m_lclptr = 0;
        else
    ...
    inline void SB_STORE(){
        ...
        DEBUG("slot: " << (int)index << " value: " << (sint)(int_fast8_t)(m_stack[m_lclptr + index].data & 0xff));           // I could just do % BYTE_MAX_VALUE but no
        m_stack.pop_back();
    }
    inline void UB_STORE(){
        ...
        DEBUG("slot: " << (int)index << " value: " << m_stack[m_lclptr + index].data);              // I could just do % BYTE_MAX_VALUE but no
        m_stack.pop_back();
    }
    inline void SI_STORE(){
        ...
        DEBUG("slot: " << (int)index << " value: " << (sint)(m_stack[m_lclptr + index].data));       
        m_stack.pop_back();
    }
    inline void UI_STORE(){
        ...
        u8 index = *getNextByte();
        m_stack[m_lclptr + index] = m_stack.back();
        DEBUG("slot: " << (int)index << " value: " << m_stack[m_lclptr + index].data);       
        m_stack.pop_back();
    }
    inline void F_STORE(){
        MNEMONIC("F_STORE");
    ...
    inline void REF_STORE(){
        ...
        u8 index = *getNextByte();
        m_stack[m_lclptr + index] = m_stack.back();
        DEBUG("slot: " << (int)index << " ref: 0x" << std::hex << m_stack[m_lclptr + index].data << std::dec);
        m_stack.pop_back();
    }
    inline void SB_LOAD(){
        MNEMONIC("SB_LOAD");
        ...
        DEBUG("slot: " << (int)index << " value: " << (m_stack.back().data & 0xff));
    }
    inline void UB_LOAD(){
        MNEMONIC("UB_LOAD");
        u8 index = *getNextByte();
    ...
    inline void SI_LOAD(){
        ...
        DEBUG("slot: " << (int)index << " value: " << m_stack.back().data);
    }
    inline void UI_LOAD(){
        MNEMONIC("UI_LOAD");
        u8 index = *getNextByte();
    ...
    inline void REF_LOAD(){
        ...
        u8 index = *getNextByte();
        m_stack.push_back(m_stack[m_lclptr + index]);
        DEBUG("slot: " << (int)index << " ref: 0x" << std::hex << m_stack.back().data << std::dec);
    }
    inline void POP(){
        MNEMONIC("POP");
        m_stack.pop_back();
